...
const ZOOM_SCALE = 5; // 이미지 버전 줌 배율
const PAN_SPEED = ZOOM_SCALE;

export default function PokemonQuizImage({ onBack }) {
  ...
  const loadPokemon = async () => {
    ...
    setPokemon({
      ...
      image: imgData.sprites.other["official-artwork"].front_default,
    });
    setGuess("");
    setFeedback("");
    setPosition({ x: 0, y: 0 });
  ...
  const moveDrag = (x, y) => {
    if (!dragging) return;
    const dx = ((x - startPos.x) / ZOOM_SCALE) * PAN_SPEED;
    const dy = ((y - startPos.y) / ZOOM_SCALE) * PAN_SPEED;
    ...
    <div className="quiz-container">
      ...
      >
        <img
          ...
          style={{
            transform: `scale(${ZOOM_SCALE}) translate(${position.x}px, ${position.y}px)`,
          }}
          draggable={false}
        />
      </div>
